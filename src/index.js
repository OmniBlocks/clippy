...
yargs(hideBin(process.argv))
  ...
  .command('lint', 'Lint this extension.', (yargs) => yargs.option('fix', {type: 'boolean'}), async (argv) => {
    await lintExtensionFiles({ fix: argv.fix });
  })

  .command('dev', 'Start the dev server.', (yargs) => {
    ...
    await startDevServer({ port: argv.port, verbose: argv.verbose, mod: argv.mod, consolaInstance });
  })
