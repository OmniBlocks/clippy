...
export const clippyPlugin = (config, blockFiles, menuFiles, hiddenBlocks, develop, target) => ({
  ...
  resolveId(id, importer) {
    ...
    if (importer && (importer.startsWith('$/clippybuilder/') || importer === '$/config') && id.startsWith('./')) {
  ...
  load(id) {
    ...
    if (subPath === 'blocks' || subPath === 'menus' || subPath === 'blocks/hidden') {
      const files = subPath === 'blocks' ? blockFiles : subPath === 'blocks/hidden' ? hiddenBlocks : menuFiles;
      const isHidden = subPath === 'blocks/hidden';
      if (files.length === 0) return `/* No ${subPath} found. */\nexport default [];`;

      const imports = files.map((file, i) => `import _m${i} from ${JSON.stringify(file)};`).join('\n');
      const registry = files.map((file, i) => {
