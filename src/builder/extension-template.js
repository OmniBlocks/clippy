...
class Extension {
  getInfo() {
    let definedBlocks = [...$$blockDefinitions, ...$$hiddenBlocks].map((b) => ({
      ...
      ...Object.fromEntries(Object.entries(b.module).filter(([k]) => k !== 'def')),
    }))
    ...
    return {
      ...
      blocks,
      menus: Object.fromEntries($$menuDefinitions.map((m) => [m.opcode, m.module])),
    }
  }
      ...
      if (isDevelop) {
        try {
          ...
          import('$/clippybuilder/devtools.js').then(({ blockErrorModal, openInEditor }) => {
            ...
            blockErrorModal(`Error in block: ${b.opcode}`, err.message, () => {
...
if (isDevelop) {
  import('$/clippybuilder/devtools.js').then(({ publish }) =>
    publish(config.id, extension, Scratch),
  )
} else {
